...
- **SFTP 里 AI 改文件**：右键远程文件 → 说改动 → 预览 diff → 写回
...
- 💾 加密备份与迁移：主机 + Keychain 密码导出为口令加密文件，新机一键导入
...
> ⚠️ SFTP 主机密钥校验目前为 `acceptAnything`（首次连接不校验 known_hosts），加密私钥(带 passphrase)与经跳板机的 SFTP 暂不支持——欢迎 PR。
...
- [ ] known_hosts 校验
- [ ] AI：会话复盘生成 Runbook、主机记忆/画像、告警自动归因
